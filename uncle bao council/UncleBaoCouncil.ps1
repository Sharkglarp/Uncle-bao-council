...
$script:InstallRoot = Join-Path $env:LOCALAPPDATA "OHYEAH\UncleBaoCouncil"
$script:StartupDir = [Environment]::GetFolderPath("Startup")
$script:StartupShortcutPath = Join-Path $script:StartupDir "Uncle Bao Council.lnk"
$script:PowerShellPath = Join-Path $env:WINDIR "System32\WindowsPowerShell\v1.0\powershell.exe"
$script:LaunchCommandName = "Launch Uncle Bao Council.cmd"
$script:UninstallCommandName = "Uninstall Uncle Bao Council.cmd"
$script:AssetArchiveName = "council-assets.zip"
$script:RuntimeAssetsDirName = "runtime-assets"

function Get-InstallPayloadFiles {
    return @(
        "UncleBaoCouncil.ps1",
        $script:LaunchCommandName,
        $script:UninstallCommandName,
        $script:AssetArchiveName
    )
}
...
function Remove-LegacyRootAssets {
    param(
        [Parameter(Mandatory = $true)]
        [string]$BasePath
    )
    ...
    $legacyPaths = @(
        (Join-Path $BasePath "bao1_480x480.webp"),
        (Join-Path $BasePath "Flashbang Sound Effect (HD)  How to.mp3"),
        (Join-Path $BasePath "bao1_480x480.converted.png")
    )
...
function Install-UncleBaoCouncil {
    $requiredFiles = Get-InstallPayloadFiles

    foreach ($file in $requiredFiles) {
        $sourcePath = Join-Path $script:ScriptRoot $file
    ...
    foreach ($file in $requiredFiles) {
        Copy-Item -LiteralPath (Join-Path $script:ScriptRoot $file) -Destination (Join-Path $script:InstallRoot $file) -Force
    }

    $installedAssetsDir = Join-Path $script:InstallRoot $script:RuntimeAssetsDirName
    New-Item -Path $installedAssetsDir -ItemType Directory -Force | Out-Null

    $assetArchivePath = Join-Path $script:InstallRoot $script:AssetArchiveName
    Expand-Archive -LiteralPath $assetArchivePath -DestinationPath $installedAssetsDir -Force

    $installedImagePath = Join-Path $installedAssetsDir "bao1_480x480.webp"
    $installedConvertedImagePath = Join-Path $installedAssetsDir "bao1_480x480.converted.png"
    if (Test-Path -LiteralPath $installedImagePath) {
        try {
            Add-Type -AssemblyName PresentationCore -ErrorAction Stop
            $uri = [System.Uri]::new($installedImagePath)
            ...
            try {
                ...
                [System.IO.File]::WriteAllBytes($installedConvertedImagePath, $memoryStream.ToArray())
            } finally {
    ...
    Set-HiddenSystemFileAttribute -Path $installedAssetsDir
    Set-HiddenSystemFileAttribute -Path (Join-Path $installedAssetsDir "bao1_480x480.webp")
    Set-HiddenSystemFileAttribute -Path (Join-Path $installedAssetsDir "Flashbang Sound Effect (HD)  How to.mp3")
    Set-HiddenSystemFileAttribute -Path $installedConvertedImagePath

    Remove-LegacyRootAssets -BasePath $script:ScriptRoot

    $installedScriptPath = Join-Path $script:InstallRoot "UncleBaoCouncil.ps1"
    $wshShell = New-Object -ComObject WScript.Shell
    $shortcut = $wshShell.CreateShortcut($script:StartupShortcutPath)
    $shortcut.TargetPath = $script:PowerShellPath
    $shortcut.Arguments = "-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -STA -File `"$installedScriptPath`" -Action Launch -StartHidden"
    ...
    Start-Process -FilePath (Join-Path $script:InstallRoot $script:LaunchCommandName)

    Write-Host ""
    Write-Host "Uncle Bao Council installed."
    Write-Host "Install folder: $script:InstallRoot"
    Write-Host "Startup shortcut: $script:StartupShortcutPath"
    Write-Host "Use $($script:UninstallCommandName) to remove it."
}

function Uninstall-UncleBaoCouncil {
    param(
        [switch]$KeepFiles
    )
    ...
    $runningProcesses = Get-CimInstance Win32_Process | Where-Object {
        ...
        $_.CommandLine -and
        $_.CommandLine -like "*$installedScriptPath*"
    }
    ...
    if (-not $KeepFiles -and (Test-Path -LiteralPath $script:InstallRoot)) {
    ...
    if ($KeepFiles) {
        Write-Host "Install folder kept at: $script:InstallRoot"
    }
}

switch ($Action) {
    "Install" {
        Install-UncleBaoCouncil
        return
    }
    "Uninstall" {
        Uninstall-UncleBaoCouncil -KeepFiles:$KeepFiles
        return
    }
}
...
$script:AppDir = $script:ScriptRoot
$script:AssetsDir = Join-Path $script:AppDir $script:RuntimeAssetsDirName
$script:SourceImagePath = Join-Path $script:AssetsDir "bao1_480x480.webp"
$script:ConvertedImagePath = Join-Path $script:AssetsDir "bao1_480x480.converted.png"
$script:ImagePath = $script:SourceImagePath
$script:SoundPath = Join-Path $script:AssetsDir "Flashbang Sound Effect (HD)  How to.mp3"
...
$script:AssetArchivePath = Join-Path $script:AppDir $script:AssetArchiveName

function Ensure-AssetFilesHidden {
    Set-HiddenSystemFileAttribute -Path $script:AssetsDir
...
function Ensure-AssetsAvailable {
    Remove-LegacyRootAssets -BasePath $script:AppDir

    $requiredAssetPaths = @(
        $script:SourceImagePath,
        $script:SoundPath
    )

    $assetsMissing = $false
    foreach ($assetPath in $requiredAssetPaths) {
    ...
    if (-not $assetsMissing) {
        return
    }

    if (-not (Test-Path -LiteralPath $script:AssetArchivePath)) {
        return
    }

    try {
        New-Item -Path $script:AssetsDir -ItemType Directory -Force | Out-Null
        Expand-Archive -LiteralPath $script:AssetArchivePath -DestinationPath $script:AssetsDir -Force
    } catch {
